...
export default {
	...
	matrixUsedCellsCount: (state, getters) => {
		...
		// If the specified number of cells is positive, we return that number (or the number of all the cells is the number is too large).
		if (specififedMatrixUsedCellsCount > 0) return Math.min(specififedMatrixUsedCellsCount, totalMatrixCellCount);

		// If the specified number of cells is positive, we return the total number of cells minus the specified number (or a minumum of 0).
		else if (specififedMatrixUsedCellsCount < 0) return Math.min(totalMatrixCellCount - Math.abs(specififedMatrixUsedCellsCount), 0);
